...
def maps(strimage):
    ...
    xc = 0
    yc = 0
    shiftRows = int(rows*0.30)
    shiftCols = int(cols*0.30)
    rows2 = rows + 2*shiftRows
    cols2 = cols + 2*shiftCols
    ...
    sm = s.copy()
    side = int((0.65*max(rows2,cols2))/2)
    centers = []
    step = int(0.15*max(rows2,cols2))
    for i in range(3):
        for j in range(3):
    ...
    for k in range(15):
        ...
        lr = max(cr-side, 0)
        hr = min(cr+side, rows2-1)
        lc = max(cc-side, 0)
        hc = min(cc+side, cols2-1)
        lrk = int(1.075*lr)
        hrk = int(0.925*hr)
        lck = int(1.075*lc)
        hck = int(0.925*hc)
